...
class UnpublishArticleRoute implements IRoute {
  path: string;
  router;
  ...
  private initRoute(): void {
    this.router.route(`${this.path}/:id/unpublish`).patch(
      (req, res, next) =>
        MiddleWares.UserMiddleWares.UserAuth.run(req, res, next),
      (req, res, next) =>
        MiddleWares.WriterMiddleWares.IsWriter.run(req, res, next),
      (req: Request, res: Response) =>
        Controllers.WriterControllers.UnpublishArticle.execute(req, res),
    );
  }
}

export default UnpublishArticleRoute;
