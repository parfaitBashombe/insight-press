...
class UpdateArticleRoute implements IRoute {
  path: string;
  router;
  ...
  private initRoute(): void {
    this.router.route(`${this.path}/:id`).patch(
      ...
      (req: Request, res: Response, next: NextFunction) =>
        Validators.ArticleValidators.Update.run(req, res, next),

      (req: Request, res: Response) =>
        Controllers.WriterControllers.UpdateArticle.execute(req, res),
    );
  }
}
