...
class CreateArticleRoute implements IRoute {
  path: string;
  router;
  ...
  private initRoute(): void {
    this.router.route(`${this.path}`).post(
      ...
      (req: Request, res: Response, next: NextFunction) =>
        Validators.ArticleValidators.Create.run(req, res, next),

      (req: Request, res: Response) =>
        Controllers.WriterControllers.CreateArticle.execute(req, res),
    );
  }
}
